...
```
We can easily visualise these differences in the plot above. The low $\beta_0$ value in the t-1 model is characterised by the smooth slope sections bisected by the introduction of the removals beginning in 1977.
...
to find that the lagged model is `r exp(AIC$AIC[1] - AIC$AIC[2])` as probable a the current model to minimise the information loss. 
