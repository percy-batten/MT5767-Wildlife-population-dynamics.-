...
We are given a species with four age classes. The dynamics of the species arise from  the sequence of sub-processes: survival, ageing, birth. To simulate population dynamics we have created a state space BAS model, which takes into consideration each of the sub-processes for each age category. From these state space model estimates, which we will consider as the true population size, we create an observation model, which considers what we would likely observe if we were to survey the population.
...
# Just a note here you've said in the previous passage that effects of rainfall in the current year are more 
# effective but then listed r_(t-1) not r(t) is this a mistake or intentional? -Percy
