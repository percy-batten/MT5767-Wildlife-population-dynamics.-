...
Everyone contributed equally to writing the code and interpreting the results. HS did questions 1 and 2(a), KA did questions 2(c) and 3(b), and PB did questions 2(b) and 3(a). We all worked together to make the final checks, edits and contributions. We read through each others and suggested any edits to be made. Overall the contributions were deemed to be equal and workload was distributed fairly among the group.
...
Wildebeest also usually breed at the end of the wet season (May-July). They have a gestation period of 240 plus or minus 11.7 days (Clay et al,2010, p365-371),thus offspring will usually be conceived at the start of the year (January-March), which makes rainfall in the current year more impact to growth as it will directly impact food supply for the young growing Wildebeest.  Also, if there is not enough rainfall, and therefore less grass growth, then they will be more likely to move in search of food and will not be observed in the current year. 

Additionally, the fact that wildebeest have an annual migration further supports the use of rainfall on the current year for use in the model as wildebeest specifically migrate in response to rainfall in the current year in relation to wet and dry seasons. Hence this will impact both the growth rate, as the population will grow if Wildebeest are migrating to the area surveyed, and carrying capacity, as the ability to migrate will depend on resources available.

Specifically wildebeest are part of the alcelaphines subfamily and it should be noted that, 
...
This supports use of current year rainfall as a better model since as mentioned earlier wet seasons are usually when the wilderbeest give birth. 

Where the current year rainfall model is given by the below equations.
...
Wildebeest have a diet that consists strictly of grass. Intuitively, this would depend on the rainfall of the current year; the effects of droughts can be very rapid and cause grass to dry out on a timescale of weeks, which may hinder the habitat's ability to support the current animal population that depends on it.  
...
```
It can be seen from the graph above both models fit well and are within error
...
```
We can easily visualise these differences in Figure 3.2. The low $\beta_0$ value in the current model is characterised by the smooth slope sections bisected by the introduction of the removals beginning in 1977. 
```{r,  include=TRUE, echo =FALSE}
rain <- ggplot(data = wildebeest, 
         ...
         scale_y_continuous(
            ...
            sec.axis = sec_axis( trans=~.*2, name="Rainfall (metres)")###metres??
        )
rain
```
Figure 3.3 gives greater insight to the how the rainfall influences the animal populations. Between the years 1970 and 1978, the area saw high average rainfalls, reflected in the change of gradient of the current model
...
AIC
```
Looking at the table above, we can see that the model with the lower AIC is the current model, when the carrying capacity is modeled by the rainfall in the current year. The AIC measures the relative goodness of fit for each model, indicating that the model for t is more likely to minimise the information loss. 
We can use the equation 

$$exp((AIC_{min} - AIC_i)/2) = exp((AIC_{t} - AIC_{t-1})/2)$$

to find that the lagged model is `r exp(AIC$AIC[1] - AIC$AIC[2])` as probable as the current model to minimise the information loss. 
