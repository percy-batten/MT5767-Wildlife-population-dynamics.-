...
print(fit)
```
Inspecting the output of coefficients as fitted, we can see that there is very little change in $N_0$ and $r$. The difference in the beta terms are much more significant:$log(K_t)$ when rain fall is equal to 0. $\beta_{0_1}$ = `r fit$par1[3]`
```{r}
#creating the 
