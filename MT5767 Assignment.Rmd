...
AIC
```
Looking at the table above, we can see that the model with the lower AIC is model 1, when the carrying capacity is modeled by the rainfall in the current year. The AIC measures the relative goodness of fit for each model, indicating that the model for t is more likely to minimise teh information loss. 
